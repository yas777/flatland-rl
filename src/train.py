...
def train_agents(args, writer):
    ...
    scores, custom_scores, completions, steps, deadlocks, frame_list = [], [], [], [], [], []
    ...
    for episode in range(args.training.train_env.episodes + 1):
        ...
        reset_timer.end()
        if args.training.renderer.training:
        ...
        for step in range(train_env._max_episode_steps):
            ...
            if args.training.renderer.training:
                ...
                #     )
                env_renderer.render_env(
                    show=False, show_observations=True, show_predictions=True
                )
        ...
        #
        print("info after end of episode")
        # train_env.print_agent_info()
        if args.training.renderer.training:
            print(f"traing done: {done}, {step}, {len(frame_list)}")
            frame_list[0].save(f"./training_gifs/flatland_multi_agent_{episode}.gif", save_all=True, append_images=frame_list[1:], duration=200, loop=1)
            frame_list=[]
...
def eval_policy(args, writer, env, policy, eval_seeds, train_episode):
    ...
    for episode, seed in enumerate(eval_seeds):
        ...
        if args.training.renderer.evaluation:
        ...
        for step in range(env._max_episode_steps):
            ...
            if done['__all__'] or env.check_if_all_blocked(info["deadlocks"]):
                print(f"done - {done}, frames - {len(frame_list)}")
                break
        if args.training.renderer.evaluation:
            gif_path = os.path.join("eval_gifs", args.run_name, str(train_episode))
            if not os.path.exists(gif_path):
                os.makedirs(gif_path)
            frame_list[0].save(os.path.join(gif_path, f"flatland_multi_agent_{episode}.gif"), save_all=True, append_images=frame_list[1:], duration=100, loop=1)
            frame_list=[]
...
def main():
    ...
    gif_dir = os.path.join("./eval_gifs", args.run_name)
